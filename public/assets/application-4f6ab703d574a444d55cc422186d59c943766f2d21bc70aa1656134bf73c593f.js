...
(function() {
  ...
  CommentsManager = (function() {
    ...
    function CommentsManager() {
      this._addComment = bind(this._addComment, this);
      this._destroyComment = bind(this._destroyComment, this);
      this._bindEvents = bind(this._bindEvents, this);
      this._bindEvents();
    }

    CommentsManager.prototype._bindEvents = function() {
      $(document).on("ajax:success", this.ui.destroyLink, this._destroyComment);
      return $(document).on("ajax:success", this.ui.commentForm, this._addComment);
    };

    CommentsManager.prototype._destroyComment = function(event) {
      var comment_id;
      comment_id = $(event.currentTarget).data("id");
      return $(this.ui.commentWrapper + "[data-id='" + comment_id + "']").remove();
    };

    CommentsManager.prototype._addComment = function(event, data, status, xhr) {
      $(this.ui.comments).append(xhr.responseText);
      return $(this.ui.commentForm).find("#comment_text").val('');
    };
